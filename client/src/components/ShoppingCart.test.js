...
test('displays plants in cart', () => {
    ...
    const firstFlowerName = screen.getByText(/peperomia rosso/i);
    const firstFlowerPrice = screen.getByText('$21');

    const secondFlowerName = screen.getByText(/string of dolphins/i);
    const secondFlowerPrice = screen.getByText('$15');

    const removeButtons = screen.getAllByText(/remove/i);
    
    expect(firstFlowerName).toBeInTheDocument();
    expect(firstFlowerPrice).toBeInTheDocument();
    expect(secondFlowerName).toBeInTheDocument();
    expect(secondFlowerPrice).toBeInTheDocument();
